...
Этот пример показывает как использовать пакет `@rci-tools/core`
для взаимодействия с RCI API. Пример написан на "ванильном" TypeScript.
Код приложения:
- аутентифицирует при помощи имени пользователя и пароля
- получает данные (`show version`) через RCI API
