// Dev-only config that proxies /auth and /rci directly to the device.
// Limitations: No ttyd/websocket terminal support (backend required).
// Use when the device serves /rci and /auth natively.
...
export default defineConfig(({mode}) => {
  ...
  if (mode === 'development') {
    ...
    const proxyAddr = `${prefix}${devAddr}`;
