...
В силу распространенности HTTP-клиентов (`curl`, `wget`, ...) и поддержки JSON
различными языками программирования и приложениями RCI удобен для автоматизации
настройки устройств Keenetic/Netcraze. Кроме того, поскольку формат JSON прекрасно
поддерживается JavaScript (и, следовательно, любым современным браузером),
RCI позволяет взаимодействовать с устройством прямо из браузера.

> При условии, что веб-сервер находится на самом устройстве или если вы настроите заголовки CORS.

RCI поддерживает семантику запросов, типичную для REST API, а также имеет ряд
...
> [!NOTE]
> Как правило, форма JSON, возвращаемая при **чтении** настроек,
> и форма JSON для **создания/изменения** этих настроек идентичны.
> Строки, целые числа и булевы значения считаются параметрами
> адресуемого ресурса API. JSON-объекты и массивы представляют **вложенные ресурсы**.
> Пустой объект (`{}`) обозначает пустой набор параметров.
...
Возможность вкладывать JSON-объекты делает RCI удобным для разработчиков.
В CLI вы обычно вводите команды по одной (**). С помощью RCI конкретная сущность
(например, сетевой интерфейс) может быть целиком представлена в виде JSON-объекта.

> ** Вы можете вставить несколько команд из буфера обмена в CLI,
> но стандартный вариант использования интерфейса командной строкиъ
...
Создание/редактирование объекта через CLI <br/>— пошаговая процедура
</td>

<td>
<img src="./svg/example-rci.svg"/><br/>
Создание/редактирование объекта через RCI <br/>может быть выполнено одним действием (HTTP-запросом)
</td>
    </tr>
</tbody>
</table>
...
### 2.2 Действия
Действия — это команды, которые не изменяют конфигурацию устройства.
Например, чтение текущей загрузки процессора, извлечение USB-накопителя
и очистка системного журнала — примеры действий. Действия выполняются мгновенно,
в отличие от [фоновых процессов](#2-3-background-processes).
...
Определенное подмножество CLI-команд является `readonly`-действиями. Ресурсы RCI,
...
но это не всегда так. Например, ресурсы `rci/whoami` или `rci/ip/http/ssl/acme/list`
являются `readonly`-ресурсами, хотя и не начинаются с `show`.
...
[Чтение конфигурации через POST-запросы](#3-reading-configuration-via-post-requests).
Такие ресурсы с префиксом также считаются действиями.


### 2.3 Фоновые процессы
Фоновые процессы — это длительные процессы, которые можно запускать,
...
описана подробнее в разделе [Фоновые процессы](#4-background-processes).
...
[Пример 2.4.3](#2-4-3-ip-telnet-nested-setting) показывает гибкость RCI API при запросе вложенного ресурса.
Там же упоминается **корневой ресурс** API (`/rci/` = `/rci` + `/`).
Корневой ресурс — это [настройка](#2-1-settings). Если вы запросите корневой ресурс
...
Тело любого RCI-запроса ограничено размером в 768 КБ. Допускается отправлять массив
объектов в качестве тела POST-запроса. В сочетании с корневым ресурсом
это дает возможность выполнять практически любые действия с устройством,
отправляя POST-запросы к корневому ресурсу (`/rci/`).
Команды, переданные в виде массива будут выполнены в том порядке, в котором они указаны в массиве.
...
[пример](#resetting-a-specific-property-within-an-object)).
